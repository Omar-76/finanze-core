...
# Tag PWA rimossi dal login per impedire il crash del manifest in RAM
...
if st.session_state.user is None:
    # Form d'ingresso semplificato senza auto-rerun distruttivo
        ...
        if email_in and pass_in:
            try:
                ...
                if res_user and len(res_user) > 0:
                    st.session_state.user = res_user[0]
                    st.session_state.nav_scelta = "🏛️ Sala di Controllo"
                    # Forza il ridisegno pulito della pagina
    ...
    ruolo_utente = str(current_user.get("role", "user")).lower()
    nome_visualizzato = f"{current_user.get('last_name', '')} {current_user.get('first_name', '')}".strip().upper()
